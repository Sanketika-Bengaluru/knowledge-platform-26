...
class TestContentActor extends BaseSpec with MockFactory {
    ...
    it should "create a node and store it in the graph" in {
        implicit val ss = mock[StorageService]
        implicit val oec: OntologyEngineContext = mock[OntologyEngineContext]
    ...
    it should "create a plugin node and store it in the graph" in {
        implicit val ss = mock[StorageService]
        implicit val oec: OntologyEngineContext = mock[OntologyEngineContext]
        val graphDB = mock[GraphService]
        (oec.graphService _).expects().returns(graphDB).anyNumberOfTimes()
   ...
   it should "return success response for 'readPrivateContent'" in {
    ...
    it should "return success response for 'updateContent'" in {
        ...
        val node = getNode()
        (graphDB.getNodeByUniqueId(_: String, _: String, _: Boolean, _: Request)).expects(*, *, *, *).returns(Future(node)).anyNumberOfTimes()
        (graphDB.getNodeProperty(_: String, _: String, _: String)).expects(*, *, *).returns(Future(new Property("versionKey", "test_123")))
    ...
    it should "return client exception for 'updateContent' with invalid versionKey" in {
        ...
        val node = getNode()
        (graphDB.getNodeByUniqueId(_: String, _: String, _: Boolean, _: Request)).expects(*, *, *, *).returns(Future(node)).anyNumberOfTimes()
        (graphDB.getNodeProperty(_: String, _: String, _: String)).expects(*, *, *).returns(Future(new Property("versionKey", "test_xyz")))
        val nodes: util.List[Node] = getCategoryNode()
        (graphDB.getNodeByUniqueIds(_: String, _: SearchCriteria)).expects(*, *).returns(Future(nodes)).anyNumberOfTimes()
...
*/
    ...
    it should "return success response for 'rejectContent'" in {
        ...
        (graphDB.getNodeByUniqueId(_: String, _: String, _: Boolean, _: Request)).expects(*, *, *, *).returns(Future(node)).anyNumberOfTimes()
        (graphDB.getNodeProperty(_: String, _: String, _: String)).expects(*, *, *).returns(Future(new Property("versionKey", "test_123"))).anyNumberOfTimes()
    ...
    it should "through client exception for publish operation if published by is not passed" in {
        ...
        val response = callActor(request, Props(new ContentActor()))
        assert(response.getResponseCode == ResponseCode.CLIENT_ERROR)
        assert(response.getParams.getErr == "ERR_CONTENT_BLANK_PUBLISHER")
        assert(response.getParams.getErrmsg == "Publisher User Id is blank")
    }
